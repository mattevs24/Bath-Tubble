...
class GameScene1: SKScene {
    ...
    func createPlayerNumbers() {
        ...
        player1.position = CGPoint(x: frame.width/6, y: frame.height*8/18)
        player1.fontSize = 36
        player1.fontName = ""
        player1.zPosition = -100
        player1.fontColor = .white
        
        player2.position = CGPoint(x: frame.width*5/6, y: frame.height*10/18)
        player2.fontSize = 36
        player2.fontName = ""
    ...
    func countdownToPlay() {
        let countdown3Texture = SKTexture(imageNamed: "Number3")
        
        countdown = SKSpriteNode(texture: countdown3Texture)
        countdown.zPosition = 100 //top layer
        countdown.size = CGSize(width: 300, height: 300)
        countdown.position = CGPoint(x: frame.width/2, y: frame.height/2)
        countdown.zRotation = .pi/2
        
        addChild(countdown)
        
        let countdown2Texture = SKTexture(imageNamed: "Number2")
        let countdown1Texture = SKTexture(imageNamed: "Number1")
        let countdownGoTexture = SKTexture(imageNamed: "Go!")
    ...
    func player1Icons(index:Int, other: Bool) {
        if other {
            ...
            var scalar = CGFloat(Double.random(in: 1...2))
            ...
            for i in 0...6 {
                let addx = 100*sin((2.0/7.0)*Double.pi*Double(i))
                let addy = 100*cos((2.0/7.0)*Double.pi*Double(i))
                icon = SKSpriteNode(imageNamed: "\(shuffled![i+1])")
                rotation = CGFloat(Double.random(in: 0...2*Double.pi))
                scalar = CGFloat(Double.random(in: 1.2...2.3))
                icon.zRotation = rotation
            ...
            icon.position = CGPoint.zero
            var rotation = CGFloat(Double.random(in: 0...2*Double.pi))
            var scalar = CGFloat(Double.random(in: 1.2...2.3))
            icon.zRotation = rotation
            icon.name = "\(shuffled![0])"
            icon.size = CGSize(width: 40*scalar, height: 40*scalar)
            //icon.zPosition = 1
